...
TEST(deriveKeyInfo, file)
{
    ...
    computeAESKeyAndIV(pin.toUtf8(), salt.toUtf8(), key, iv);

    QPair<QByteArray, QByteArray> produced {key, iv};
    QPair<QByteArray, QByteArray> expected {QByteArray::fromHex(derivedKey.toUtf8()), QByteArray::fromHex(derivedIV.toUtf8())};
...
TEST(encStr, file)
{
    ...
    QString derivedKey = "b340947333ac4783f8d88683acc45f0165834e14160df964ffa5d3aee6b9c8dc";
    QString derivedIV = "65834e14160df964ffa5d3aee6b9c8dc";
    QByteArray key = QByteArray::fromHex(derivedKey.toUtf8());
    QByteArray iv = QByteArray::fromHex(derivedIV.toUtf8());
